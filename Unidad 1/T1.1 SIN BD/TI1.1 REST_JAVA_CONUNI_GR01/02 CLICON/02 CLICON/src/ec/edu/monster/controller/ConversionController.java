...
public class ConversionController {
    ...
    // Ajusta si cambia el contexto del WAR o el puerto:
    private static final String BASE_URL =
            "http://localhost:8080/WS_CONUNI_RESTJAVA_GRO01/api/conversion/";

    // Extrae el primer número (p.ej. si la respuesta es JSON como {"valor": 6.21371})
    private static double parseNumber(String body) {
        // 1) intento directo (respuesta plana "6.21371")
        try {
            return Double.parseDouble(body.trim());
        } catch (Exception ignore) {}

        // 2) buscar primer número con regex (sirve para JSON o texto)
    ...
    private String getBody(String endpoint) {
        try {
            ...
            // Acepta cualquier tipo para evitar 406
